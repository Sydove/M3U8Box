...
`M3U8Box` 是一个基于 Go 的命令行工具，用来从页面中提取 `m3u8` 播放地址，下载加密密钥和 `ts` 分片，并通过 `ffmpeg` 合并为本地 `mp4` 文件。同时也支持把本地 `mp4` 切分为 `m3u8 + ts` 的 HLS 文件集。
...
```bash
./M3U8Box -i="https://example.com/video-page" [-d="<目录>"] [-n="<文件名>"] [-c=10]
./M3U8Box -f="./links.txt" [-d="<目录>"] [-n="<文件名>"] [-c=10]
```

#### 生成模式

```bash
./M3U8Box -mp4="/Users/you/Videos/demo.mp4" [-d="<目录>"] [-n="<名称>"] [-hls-time=10]
```
...
| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `-mp4` | 本地 MP4 文件路径，用于生成 HLS | 无 |
| `-hls-time` | HLS 切片时长（秒） | `10` |
...
| --- | --- | --- |
| `-d` | 输出目录 | 当前工作目录 |
| `-n` | 输出名称；下载模式默认使用页面标题，生成模式默认使用 MP4 文件名 | 自动推导 |
...
```bash
./M3U8Box -mp4="/Users/you/Videos/demo.mp4" -d="/Users/you/Exports" -n="demo-hls" -hls-time=8
```
...
- `static/` 临时目录
- HLS 输出目录，例如 `demo-hls/index.m3u8` 与配套 `segment_*.ts`
